...
from ..deps import SessionDep, ActiveUserDep
...
def create_user(user: UserCreate, session: SessionDep):
    ...
    user_in = get_user_by_email(session=session, email=user.email)

    #TODO add logic to check if user is disabled
    if user_in:
        raise HTTPException(
    ...
    #TODO check if user.email == user.password:
